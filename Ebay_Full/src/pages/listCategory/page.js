...
export default function ListCategory() {
  // Extract categoryId from URL params but don't parse as integer
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Find category by id without parsing to number
        if (categoryId) {
          // Option 1: Find from local categories
          ...
          if (currentCategory) {
            ...
            // Option 2: Directly fetch the category data from endpoint
            const categoryDataResponse = await fetch(
              `http://localhost:9999/list-category/${categoryId}`
            );

            if (categoryDataResponse.ok) {
              const categoryData = await categoryDataResponse.json();
              setCategory(categoryData.category);
              setProducts(categoryData.products || []); // Ensure products is always an array
            } else {
  ...
  const getSubcategories = () => {
    return categorySubcategories[categoryId] || categorySubcategories.default;
  };

  // Get filters based on current category
  const getFilters = () => {
    return categoryFilters[categoryId] || categoryFilters.default;
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 max-w-3xl">
            Browse our selection of {category.name.toLowerCase()} products. Find
            great deals on top brands and quality items.
          </p>
        </div>
            ...
            <div className="fixed inset-y-0 left-0 max-w-xs w-full bg-white shadow-xl p-6 overflow-y-auto">
              ...
              <div className="divide-y divide-gray-200">
                ...
                {getFilters().map((filter, filterIndex) => (
                  <div key={`mobile-filter-${filterIndex}`} className="py-6">
                    <h3 className="font-medium text-gray-900">{filter.name}</h3>
                    <ul className="mt-4 space-y-2">
                      {filter.options.map((option, optionIndex) => (
                        <li
                          key={`mobile-option-${filterIndex}-${optionIndex}`}
                          className="flex items-center"
                        >
                          <input
                          ...
                          />
                          <label
                            htmlFor={`mobile-${filter.name}-${optionIndex}`}
          ...
          <div className="hidden lg:block">
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-6">
              ...
              <div className="mt-8 pt-6 border-t border-gray-200">
                ...
                {getFilters().map((filter, filterIndex) => (
                  <div key={`desktop-filter-${filterIndex}`} className="mb-6">
                    ...
                    <div className="space-y-2">
                      {filter.options.map((option, optionIndex) => (
                        <div
                          key={`desktop-option-${filterIndex}-${optionIndex}`}
                          className="flex items-center"
                        >
                          <input
                          ...
                          />
                          <label
                            htmlFor={`desktop-${filter.name}-${optionIndex}`}
                            className="ml-3 text-gray-600"
                          >
                            {option}
                          </label>
                    ...
                    {Object.entries(activeFilters).map(
                      ([filterName, options]) =>
                        options.map((option, index) => (
                          <span
                            key={`active-filter-${filterName}-${index}`}
